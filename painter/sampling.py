"""Gradient-oriented stroke sampling for Phase 0 painters."""
...
def sample_gradient_strokes(
    ...
    *,
    ...
    if weight_map is None:
        ...
        if weight_map.shape != magnitude.shape:
            raise ValueError("weight_map must have shape (H, W)")
        if not np.isfinite(weight_map).all() or np.any(weight_map < 0.0):
            raise ValueError("weight_map must be finite and non-negative")
        weights_2d = weight_map.astype(float, copy=True)
        if float(weights_2d.sum()) <= 0.0:
            weights_2d = magnitude.astype(float)
            weights_2d += max(float(weights_2d.mean()) * 0.15, 1e-8)
